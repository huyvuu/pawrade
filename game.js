...
var arrivalScene = null, denDoorAjar = 0;   // the arrival ceremony: a scared newcomer slips in from the cold
...
// ---- TYPED TILES: the complexity staircase. cozy house obstacles that mostly create CHOICES, not walls. ----
// quilt(n): passable, folded away by a merge touching it · yarn(n): blocks until unravelled by an adjacent merge
// moon: passable, a merge ENDING here scores double · draft: passable, a pet landing here dozes off
// spawn(b): a toybox that occupies its cell and keeps spilling its breed into the room
var curTiles = {};
function tkey(x, y) { return x + ',' + y; }
function tilesFromSpec(spec) { var m = {}, a = (spec && spec.tiles) || [], i; for (i = 0; i < a.length; i++) m[tkey(a[i].x, a[i].y)] = { t: a[i].t, n: a[i].n, b: a[i].b }; return m; }
function tileBlocks(tm, x, y) { var t = tm[tkey(x, y)]; return !!(t && (t.t === 'yarn' || t.t === 'spawn') && (t.n === undefined || t.n > 0)); }
var FOLD_D = [[0, 0], [1, 0], [-1, 0], [0, 1], [0, -1]];
function foldTiles(tm, cells) {   // a hug tidies the quilt / unravels the yarn it touches (and its orthogonal neighbours)
  var i, d, k, folded = 0, hit = {};
  for (i = 0; i < cells.length; i++) for (d = 0; d < 5; d++) hit[tkey(cells[i][0] + FOLD_D[d][0], cells[i][1] + FOLD_D[d][1])] = 1;
  for (k in hit) {   // ONE layer per hug, however many of its cells touched the tile
    var t = tm[k];
    if (t && (t.t === 'quilt' || t.t === 'yarn')) { t.n = (t.n || 1) - 1; folded++; if (t.n <= 0) delete tm[k]; }
  }
  return folded;
}
function tilesLeft(tm, type) { var c = 0, k; for (k in tm) if (tm[k].t === type) c++; return c; }
function isBlockedCell(x, y) { return isMaskedCell(curMask, x, y) || tileBlocks(curTiles, x, y); }
...
function computeBotPar(seedStr, spec) {
  ...
  var mask = spec.mask || [];
  var btiles = tilesFromSpec(spec);                                   // the bot faces the same quilts/yarn/moonbeams/drafts
  function bBlocked(x, y) { return isMaskedCell(mask, x, y) || tileBlocks(btiles, x, y); }
  var g = [], sleep = [];
  for (var y = 0; y < ROWS; y++) { g.push([]); sleep.push([]); for (var x = 0; x < COLS; x++) { var blk = bBlocked(x, y); g[y].push(blk ? null : (bp < bdeck.length ? bdeck[bp++] : null)); sleep[y].push(false); } }
  if (spec.presetSleep) { // same fixed opening shape the real board uses, so the bot faces the identical architecture
    ...
    var openCells2 = []; for (var ay2 = 0; ay2 < ROWS; ay2++) for (var ax2 = 0; ax2 < COLS; ax2++) if (!bBlocked(ax2, ay2)) openCells2.push([ax2, ay2]);
  ...
  while (guard++ < 400) {
    ...
    if (kind.s >= 3) { for (var ci = 0; ci < n; ci++) g[cells[ci][1]][cells[ci][0]] = null; foldTiles(btiles, cells); }
    else {
      var end = cells[n - 1], bmt = btiles[tkey(end[0], end[1])];
      var bGain = 12 * n * n * STAGE_MULT[kind.s];
      if (bmt && bmt.t === 'moon') bGain *= 2;          // the bot can use the moonlight too
      score += bGain;
      for (var cj = 0; cj < n; cj++) g[cells[cj][1]][cells[cj][0]] = null;
      g[end[1]][end[0]] = { b: kind.b, s: kind.s + 1 };
      foldTiles(btiles, cells);
      merges++;
    ...
    for (var gx = 0; gx < COLS; gx++) {
      ...
      for (var gy = ROWS - 1; gy >= 0; gy--) {
        if (bBlocked(gx, gy)) { for (var fy2 = wy; fy2 > gy; fy2--) { g[fy2][gx] = bp < bdeck.length ? bdeck[bp++] : null; sleep[fy2][gx] = false; } wy = gy - 1; continue; }
    ...
    for (var tk in btiles) {                        // drafts doze whoever settled in them; toyboxes spill (same fixed order as the board)
      var bt = btiles[tk], tp = tk.split(','), tx = +tp[0], ty = +tp[1];
      if (bt.t === 'draft') { if (g[ty] && g[ty][tx]) sleep[ty][tx] = true; }
      else if (bt.t === 'spawn' && bt.b) {
        for (var sd = 1; sd < FOLD_D.length; sd++) {
          var ux = tx + FOLD_D[sd][0], uy = ty + FOLD_D[sd][1];
          if (ux < 0 || ux >= COLS || uy < 0 || uy >= ROWS) continue;
          if (g[uy][ux] && !sleep[uy][ux] && g[uy][ux].s === 1 && g[uy][ux].b !== bt.b && g[uy][ux].b !== 'voidcat') { g[uy][ux] = { b: bt.b, s: 1 }; break; }
        }
      }
    }
    if (merges % YE === 0) { // yawn
...
function onDown(e) {
  ...
  if (arrivalScene) {   // tap anywhere skips the ceremony straight to the "leave the door open?" prompt
    if (!arrivalScene.prompted) { arrivalScene.t0 = performance.now() - ARR_HIDE; }
    return;
  }
  ...
  if (screenMode === 'den') {
    ...
    if (curRoom === 0) {
      ...
      if (curFeaturedRescue >= 0 && Math.hypot(p.x - rescueScreenPos.x, p.y - rescueScreenPos.y) < rescueScreenPos.r * 1.25) { openRescueMenu(curFeaturedRescue); return; }
      return;
    }
...
function resolveMerge() {
  ...
  if (isFinal) {
    ...
    persist();
    var gCells = []; for (var gk = 0; gk < n; gk++) gCells.push([path[gk].x, path[gk].y]);
    foldTiles(curTiles, gCells);   // a graduation tidies what it walks past too
    ...
    var moonT = curTiles[tkey(endPos.x, endPos.y)];      // a hug finished in the moonlight is worth double
    if (moonT && moonT.t === 'moon') { gained *= 2; callout('in the moonlight ×2', '#a8d8ff'); }
    var pathCells = []; for (var pcI = 0; pcI < n; pcI++) pathCells.push([path[pcI].x, path[pcI].y]);
    if (foldTiles(curTiles, pathCells)) noisePuff(0, 0.07, 520);   // quilts fold, yarn unravels — a soft rustle
...
// ---- tile art: cozy house obstacles, drawn in the room's own language ----
function drawQuiltTile(cx, cy, n) {
  ctx.save();
  ctx.fillStyle = n > 1 ? 'rgba(152,112,172,.5)' : 'rgba(192,152,202,.36)';
  roundRect(cx - CELL / 2 + 5, cy - CELL / 2 + 5, CELL - 10, CELL - 10, 14); ctx.fill();
  ctx.strokeStyle = 'rgba(255,220,240,.32)'; ctx.lineWidth = 1.5;
  if (ctx.setLineDash) ctx.setLineDash([5, 4]);
  roundRect(cx - CELL / 2 + 11, cy - CELL / 2 + 11, CELL - 22, CELL - 22, 10); ctx.stroke();
  if (ctx.setLineDash) ctx.setLineDash([]);
  ctx.restore();
}
function drawMoonTile(cx, cy, now) {
  var pulse = 0.15 + 0.09 * Math.sin(now / 700);
  var mg = ctx.createRadialGradient(cx, cy, 4, cx, cy, CELL * 0.62);
  mg.addColorStop(0, 'rgba(168,216,255,' + (pulse + 0.16) + ')'); mg.addColorStop(1, 'rgba(168,216,255,0)');
  ctx.fillStyle = mg; ctx.beginPath(); ctx.arc(cx, cy, CELL * 0.62, 0, 7); ctx.fill();
}
function drawDraftTile(cx, cy, now) {
  ctx.save(); ctx.globalAlpha = 0.45; ctx.strokeStyle = '#7fb8e8'; ctx.lineWidth = 2; ctx.lineCap = 'round';
  for (var i = 0; i < 3; i++) {
    var yy = cy - 13 + i * 13, ph = Math.sin(now / 520 + i) * 6;
    ctx.beginPath(); ctx.moveTo(cx - 16 + ph, yy); ctx.quadraticCurveTo(cx + ph, yy - 5, cx + 16 + ph, yy); ctx.stroke();
  }
  ctx.restore();
}
function drawYarnTile(cx, cy, n) {
  ctx.fillStyle = 'rgba(40,28,58,.5)'; roundRect(cx - CELL / 2 + 6, cy - CELL / 2 + 6, CELL - 12, CELL - 12, 16); ctx.fill();
  ctx.fillStyle = n > 1 ? '#b8607e' : '#e0899f'; ctx.beginPath(); ctx.arc(cx, cy, CELL * 0.25, 0, 7); ctx.fill();
  ctx.strokeStyle = 'rgba(255,255,255,.32)'; ctx.lineWidth = 2;
  for (var a = 0; a < 3; a++) { ctx.beginPath(); ctx.arc(cx, cy, CELL * 0.25 - 3, a * 2.1, a * 2.1 + 1.6); ctx.stroke(); }
  if (n > 1) { ctx.fillStyle = '#f6efe6'; ctx.font = "700 12px 'Baloo 2', sans-serif"; ctx.textAlign = 'center'; ctx.fillText(n, cx, cy + CELL * 0.44); ctx.textAlign = 'left'; }
}
function drawToyboxTile(cx, cy) {
  ctx.fillStyle = '#5f8fc0'; roundRect(cx - CELL / 2 + 9, cy - CELL / 2 + 16, CELL - 18, CELL - 26, 9); ctx.fill();
  ctx.fillStyle = '#77a8d8'; roundRect(cx - CELL / 2 + 7, cy - CELL / 2 + 9, CELL - 14, 12, 6); ctx.fill();
  ctx.fillStyle = '#ffca7a'; ctx.beginPath(); ctx.arc(cx - 8, cy - CELL * 0.28, 6, 0, 7); ctx.fill();
  ctx.fillStyle = '#e8879e'; ctx.beginPath(); ctx.arc(cx + 8, cy - CELL * 0.24, 5, 0, 7); ctx.fill();
}
// the cold draft dozes whoever settles in it; the toybox keeps spilling its own kind into the room
function applyTileEffects() {
  ...
  for (k in curTiles) {
    ...
    if (t.t === 'draft') {
      ...
      var c = grid[y] && grid[y][x];
      if (c && c.pet && !c.asleep) { c.asleep = true; }
    }
  }
  for (k in curTiles) {
    t = curTiles[k];
    if (t.t === 'spawn' && t.b) {
      var q = k.split(','), sx = +q[0], sy = +q[1], near = [], d;
      for (d = 1; d < FOLD_D.length; d++) {   // skip [0,0] — the toybox spills into the room, not onto itself
        var nx = sx + FOLD_D[d][0], ny = sy + FOLD_D[d][1];
        if (nx < 0 || nx >= COLS || ny < 0 || ny >= ROWS) continue;
        var nc = grid[ny][nx];
        if (nc && nc.pet && !nc.asleep && nc.pet.stage === 1 && nc.pet.breed !== t.b && nc.pet.breed !== 'voidcat') near.push(nc);
      }
      if (near.length) { var pick = near[0]; pick.pet = { breed: t.b, stage: 1, coat: pick.pet.coat || 0 }; }   // fixed scan order — the daily must stay identical worldwide (law #2)
    }
  }
}
...
function showEnd(d) {
  ...
  if (d.daily && pendingArrivalProfile) {
    ...
    setTimeout(function () { hideOv('end-ov'); goDen(); startArrival(arrivingProfile); }, 1400);
  }
}
function shareText() {
...
// ---- THE ARRIVAL CEREMONY: a scared newcomer slips in from the cold (3 beats, ≤6s, skippable) ----
function easeOutCubic(k) { return 1 - Math.pow(1 - k, 3); }
function drawDoor(now) {
  var dw = 56, dh = 108, dx = ISO_CX - dw / 2, dy = ISO_TOPY - dh, aj = denDoorAjar;
  ctx.fillStyle = '#211830'; roundRect(dx - 5, dy - 6, dw + 10, dh + 6, 9); ctx.fill();     // frame
  var ig = ctx.createLinearGradient(dx, dy, dx, dy + dh); ig.addColorStop(0, aj > 0.02 ? '#4a3722' : '#191122'); ig.addColorStop(1, aj > 0.02 ? '#2a1c10' : '#120c1c');
  ctx.fillStyle = ig; roundRect(dx, dy, dw, dh, 6); ctx.fill();                              // doorway interior
  if (aj > 0.02) {                                                                           // warm light spills onto the floor
    ctx.save(); ctx.globalAlpha = 0.3 * aj; ctx.fillStyle = '#ffca7a';
    ctx.beginPath(); ctx.moveTo(ISO_CX - 8, ISO_TOPY); ctx.lineTo(ISO_CX + 8, ISO_TOPY); ctx.lineTo(ISO_CX + 74 * aj, ISO_TOPY + 130 * aj); ctx.lineTo(ISO_CX - 74 * aj, ISO_TOPY + 130 * aj); ctx.closePath(); ctx.fill(); ctx.restore();
  }
  var pw = dw * (1 - aj * 0.82);                                                             // the panel, swung ajar
  ctx.fillStyle = '#382a50'; ctx.beginPath(); ctx.moveTo(dx, dy); ctx.lineTo(dx + pw, dy + 7 * aj); ctx.lineTo(dx + pw, dy + dh - 7 * aj); ctx.lineTo(dx, dy + dh); ctx.closePath(); ctx.fill();
  ctx.strokeStyle = '#4a3868'; ctx.lineWidth = 1.5; ctx.stroke();
  ctx.fillStyle = '#ffca7a'; ctx.beginPath(); ctx.arc(dx + pw - 8, dy + dh * 0.55, 3, 0, 7); ctx.fill();
}
function drawScaredSilhouette(x, y, sc) {
  var rr = 44 * sc;
  ctx.fillStyle = 'rgba(13,8,21,.4)'; ctx.beginPath(); ctx.ellipse(x, y + rr * 0.12, rr * 0.66, rr * 0.15, 0, 0, 7); ctx.fill();
  ctx.fillStyle = '#1b1327'; ctx.beginPath(); ctx.ellipse(x, y - rr * 0.5, rr * 0.68, rr * 0.72, 0, 0, 7); ctx.fill();   // dark body
  ctx.beginPath(); ctx.moveTo(x - rr * 0.5, y - rr * 0.86); ctx.lineTo(x - rr * 0.32, y - rr * 1.28); ctx.lineTo(x - rr * 0.1, y - rr * 0.92); ctx.closePath(); ctx.fill();
  ctx.beginPath(); ctx.moveTo(x + rr * 0.5, y - rr * 0.86); ctx.lineTo(x + rr * 0.32, y - rr * 1.28); ctx.lineTo(x + rr * 0.1, y - rr * 0.92); ctx.closePath(); ctx.fill();
  ctx.fillStyle = 'rgba(255,224,150,.92)';                                                   // two scared glints
  ctx.beginPath(); ctx.arc(x - rr * 0.22, y - rr * 0.56, rr * 0.1, 0, 7); ctx.fill();
  ctx.beginPath(); ctx.arc(x + rr * 0.22, y - rr * 0.56, rr * 0.1, 0, 7); ctx.fill();
}
function startArrival(profile) {
  arrivalScene = { profile: profile, t0: performance.now(), lastScratch: 0, w1: false, w2: false, prompted: false };
  denDoorAjar = 0; setUI();
}
var ARR_B1 = 2000, ARR_B2 = 2700, ARR_DASH = 4300, ARR_HIDE = 4700;   // cumulative ms
function renderArrival(now) {
  var a = arrivalScene, e = now - a.t0, hp = denProject(0.31, 0.44);
  if (e < 350 && !a.w1) { a.w1 = true; setWhisper('someone’s outside…'); }
  if (e < ARR_B2) denDoorAjar = Math.min(0.16, (e / ARR_B1) * 0.16);
  else denDoorAjar = 0.16 + 0.84 * easeOutCubic(Math.min(1, (e - ARR_B2) / (ARR_DASH - ARR_B2)));
  if (e < ARR_DASH && now - a.lastScratch > 600) { a.lastScratch = now; if (e < ARR_B2) noisePuff(0, 0.08, 300 + Math.random() * 220); }
  if (e >= ARR_B2 && e < ARR_DASH) {                                    // the dash from door to hiding spot
    var k = (e - ARR_B2) / (ARR_DASH - ARR_B2), ek = easeOutCubic(k);
    var sx = ISO_CX + (hp.x - ISO_CX) * ek, sy = (ISO_TOPY + 8) + (hp.y - (ISO_TOPY + 8)) * ek;
    var bob = Math.abs(Math.sin(k * Math.PI * 5)) * 9 * (1 - k * 0.5);
    drawScaredSilhouette(sx, sy - bob, 0.62 + 0.38 * ek);
    if (e > ARR_B2 + 220 && !a.w2) { a.w2 = true; setWhisper('oh — a little one, quick and scared.'); }
  } else if (e >= ARR_DASH) {                                           // hiding, trembling
    drawScaredSilhouette(hp.x + Math.sin(now / 70) * 1.3, hp.y, 1);
    if (!a.prompted && e >= ARR_HIDE) { a.prompted = true; setTimeout(function () { if (arrivalScene) showRescueArrival(arrivalScene.profile); }, 300); }
  }
  if (e > 800 && e < ARR_DASH) { ctx.textAlign = 'right'; ctx.font = "500 15px Fredoka, sans-serif"; ctx.fillStyle = 'rgba(185,169,201,.65)'; ctx.fillText('tap to skip', W - 24, 152); ctx.textAlign = 'left'; }
}
function showRescueArrival(profile) {
  var rc = $('rescue-cv').getContext('2d');
  var bg = rc.createLinearGradient(0, 0, 0, 200); bg.addColorStop(0, '#241b3e'); bg.addColorStop(1, '#2e2247');
  rc.fillStyle = bg; rc.fillRect(0, 0, 280, 200);
  // a warm doorway glow behind her — she came in from the cold
  var dg = rc.createRadialGradient(140, 60, 8, 140, 60, 120); dg.addColorStop(0, 'rgba(255,210,130,.22)'); dg.addColorStop(1, 'rgba(255,180,90,0)');
  rc.fillStyle = dg; rc.fillRect(0, 0, 280, 200);
  // the crate she's peeking around
  rc.fillStyle = '#8a6a48'; rc.beginPath(); rc.moveTo(30, 132); rc.lineTo(44, 190); rc.lineTo(150, 190); rc.lineTo(164, 132); rc.closePath(); rc.fill();
  rc.fillStyle = '#a58057'; rc.fillRect(24, 124, 146, 13);
  drawPet(rc, profile.breed, profile.coat, 186, 118, 50, { sad: true, wet: true, pupil: { x: -0.7, y: 0.4 } });   // peeking from the right of the crate
  $('rescue-head').textContent = 'someone came in from the cold';
  $('rescue-line').textContent = profile.found;
  $('rescuename').value = profile.suggest;
  $('btn-adopt').innerHTML = '🚪 leave the door open for her';
  showOv('rescue-ov');
}
$('btn-adopt').addEventListener('click', function () {
  var profile = (arrivalScene && arrivalScene.profile) || pendingArrivalProfile || RESCUE_POOL[0];
  ...
  pendingArrivalProfile = null; arrivalScene = null; denDoorAjar = 0;
  save.arrivalsSeen = (save.arrivalsSeen || 0) + 1;
  persist(); audio();
  mallet(SCALE_HZ[0], 0, 0.14, 0.5); mallet(SCALE_HZ[4], 0.15, 0.16, 0.6); mallet(SCALE_HZ[8], 0.32, 0.16, 1.0);
  hideOv('rescue-ov');
  goDen();
  setWhisper('you left the door open. ' + nm + ' stayed — still shy, but she stayed.');
});
...
// ---- THE COAX RITUAL: four gentle gestures, one each per day, one is her secret favorite ----
var RESCUE_VERBS = ['sit', 'treat', 'words', 'toy'];
var RESCUE_VERB_LABEL = { sit: 'sit near', treat: 'a treat', words: 'soft words', toy: 'roll a toy' };
var RESCUE_FAV = { tux: 'words', tabby: 'toy', voidcat: 'sit', corgi: 'treat', shiba: 'toy', frenchie: 'treat' };
function rescueFavVerb(r) { return r.trait === 'cart' ? 'toy' : r.trait === 'patch' ? 'words' : (RESCUE_FAV[r.breed] || 'treat'); }
var rescueVerbMenu = null, verbChipRects = [];
function drawTinyHeart(x, y, s) { ctx.beginPath(); ctx.moveTo(x, y + s * 0.3); ctx.bezierCurveTo(x - s, y - s * 0.7, x - s * 1.5, y + s * 0.45, x, y + s * 1.25); ctx.bezierCurveTo(x + s * 1.5, y + s * 0.45, x + s, y - s * 0.7, x, y + s * 0.3); ctx.closePath(); ctx.fill(); }
function drawVerbIcon(v, x, y, s) {
  ctx.save(); ctx.translate(x, y);
  if (v === 'sit') { ctx.fillStyle = '#7290b2'; ctx.beginPath(); ctx.ellipse(0, s * 0.3, s * 0.72, s * 0.34, 0, 0, 7); ctx.fill(); ctx.fillStyle = '#96b0d0'; ctx.beginPath(); ctx.ellipse(0, s * 0.15, s * 0.58, s * 0.26, 0, 0, 7); ctx.fill(); }
  else if (v === 'treat') { ctx.fillStyle = '#e0a060'; ctx.save(); ctx.rotate(0.5); ctx.beginPath(); ctx.ellipse(0, 0, s * 0.62, s * 0.24, 0, 0, 7); ctx.fill(); ctx.beginPath(); ctx.arc(-s * 0.55, -s * 0.2, s * 0.2, 0, 7); ctx.fill(); ctx.beginPath(); ctx.arc(-s * 0.55, s * 0.2, s * 0.2, 0, 7); ctx.fill(); ctx.beginPath(); ctx.arc(s * 0.55, -s * 0.2, s * 0.2, 0, 7); ctx.fill(); ctx.beginPath(); ctx.arc(s * 0.55, s * 0.2, s * 0.2, 0, 7); ctx.fill(); ctx.restore(); }
  else if (v === 'words') { ctx.fillStyle = '#e8b4c8'; roundRect(-s * 0.7, -s * 0.55, s * 1.4, s * 0.9, 6); ctx.fill(); ctx.beginPath(); ctx.moveTo(-s * 0.25, s * 0.3); ctx.lineTo(-s * 0.05, s * 0.66); ctx.lineTo(s * 0.15, s * 0.3); ctx.closePath(); ctx.fill(); ctx.fillStyle = '#33244e'; for (var d = -1; d <= 1; d++) { ctx.beginPath(); ctx.arc(d * s * 0.34, -s * 0.1, s * 0.09, 0, 7); ctx.fill(); } }
  else { ctx.fillStyle = '#7fb8e8'; ctx.beginPath(); ctx.arc(0, 0, s * 0.56, 0, 7); ctx.fill(); ctx.strokeStyle = '#5a94c4'; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(0, 0, s * 0.38, 0.4, 2.6); ctx.stroke(); ctx.beginPath(); ctx.arc(0, 0, s * 0.38, 3.5, 5.8); ctx.stroke(); }
  ctx.restore();
}
function openRescueMenu(idx) {
  var r = save.rescues[idx]; if (!r) return; audio();
  if (r.vday !== dayNumber()) { r.vday = dayNumber(); r.vused = {}; persist(); }
  var allUsed = true, vi; for (vi = 0; vi < 4; vi++) if (!(r.vused && r.vused[RESCUE_VERBS[vi]])) allUsed = false;
  if (allUsed) { setWhisper('she’s had her fill of kindness today. tomorrow?'); return; }
  rescueVerbMenu = { idx: idx }; setUI();
}
function applyVerb(idx, verb) {
  var r = save.rescues[idx]; if (!r) return;
  if (r.vday !== dayNumber()) { r.vday = dayNumber(); r.vused = {}; }
  if (r.vused && r.vused[verb]) return;
  audio(); r.vused = r.vused || {}; r.vused[verb] = true;
  var fav = rescueFavVerb(r) === verb, before = rescueStage(r.trust);
  r.trust = Math.min(100, r.trust + (fav ? 12 : 4));
  var after = rescueStage(r.trust);
  r._pop = performance.now();
  rescueVerbMenu = null; persist(); setUI();
  var rp = rescueScreenPos;
  if (fav) { burstHeartsAt(rp.x, rp.y - 20, 11); mallet(SCALE_HZ[4], 0, 0.12, 0.3); mallet(SCALE_HZ[7], 0.13, 0.13, 0.5); }
  else { burstHeartsAt(rp.x, rp.y - 20, 4); mallet(SCALE_HZ[2], 0, 0.08, 0.22); }
  if (after > before) {
    var cross = ['', 'she stops hiding now — just watches you.', 'she took a step closer. toward you.', ''][after];
    setWhisper(cross || 'braver, a little, every day.'); burstHeartsAt(rp.x, rp.y - 20, 12);
  } else if (fav) {
    var fl = { sit: r.name + ' settles beside you. that’s new.', treat: r.name + ' sniffs — then, quietly, nibbles.', words: 'an ear turns toward your voice and stays.', toy: r.name + ' bats the toy once. just once. but still.' };
    setWhisper(fl[verb]);
  } else {
...
function drawRescueMenu(now) {
  ...
  verbChipRects = [];
  ctx.fillStyle = 'rgba(13,8,21,.74)'; ctx.fillRect(0, 0, W, H);
  ctx.textAlign = 'center';
  ctx.font = "700 24px 'Baloo 2', sans-serif"; ctx.fillStyle = '#ffd9a0';
  ctx.fillText('how will you reach ' + r.name + '?', W / 2, 316);
  ctx.font = "500 16px Fredoka, sans-serif"; ctx.fillStyle = '#b9a9c9';
  ctx.fillText('one gesture a day — she remembers which', W / 2, 346);
  var cw = 202, ch = 90, gap = 16, x0 = W / 2 - cw - gap / 2, y0 = 392;
  for (var i = 0; i < 4; i++) {
    var v = RESCUE_VERBS[i], used = !!(r.vused && r.vused[v]);
    var cx = x0 + (i % 2) * (cw + gap), cy = y0 + Math.floor(i / 2) * (ch + gap);
    ctx.fillStyle = used ? 'rgba(42,29,61,.5)' : 'rgba(42,29,61,.96)'; roundRect(cx, cy, cw, ch, 16); ctx.fill();
    ctx.strokeStyle = used ? '#3a2f4a' : '#574371'; ctx.lineWidth = 1.5; roundRect(cx, cy, cw, ch, 16); ctx.stroke();
    ctx.save(); ctx.globalAlpha = used ? 0.4 : 1;
    drawVerbIcon(v, cx + 42, cy + ch / 2 - (used ? 8 : 0), 24);
    ctx.textAlign = 'left'; ctx.font = "700 19px 'Baloo 2', sans-serif"; ctx.fillStyle = '#f6efe6';
    ctx.fillText(RESCUE_VERB_LABEL[v], cx + 74, cy + ch / 2 + (used ? -2 : 6));
    if (used) { ctx.font = "500 12px Fredoka, sans-serif"; ctx.fillStyle = '#8fd0a4'; ctx.fillText('given today ✓', cx + 74, cy + ch / 2 + 18); }
    ctx.restore();
    verbChipRects.push({ x: cx, y: cy, w: cw, h: ch, v: v, used: used });
  }
  ...
  if (save.rescues.length) {
    ...
    (function (rx, ry, rsc) {
      jobs.push({ depth: ry, draw: function () {
        ...
        var pcy = ry - rr * 0.5;
        if (featR._pop && now - featR._pop < 500) { var pp = (now - featR._pop) / 500, ps = 1 + 0.17 * Math.sin(pp * Math.PI); ctx.translate(rx, pcy); ctx.scale(ps, ps); ctx.translate(-rx, -pcy); }
        drawPet(ctx, featR.breed, featR.coat, rx, pcy, rr, { breath: s2.breath(now), sleeping: rstage === 0, sad: rstage === 1, pupil: rstage <= 1 ? { x: 0.55, y: 0.15 } : { x: 0, y: 0 } });
        drawTraitMark(ctx, featR.trait, rx, pcy, rr, rstage < 3);
        ...
        ctx.restore();
        var topY = ry - rr * 1.85;                                   // above her ears
        ctx.font = "600 15px 'Baloo 2', sans-serif"; ctx.fillStyle = '#c9b8d8'; ctx.textAlign = 'center';
        ctx.fillText(featR.name + ' · ' + RESCUE_STAGE_WORD[rstage], rx, topY - 8);
        if (rstage < 3) {                                            // the visible trust meter, filling in warm
          var barW = 64, barH = 8, barX = rx - barW / 2;
          ctx.fillStyle = 'rgba(30,20,44,.85)'; roundRect(barX, topY, barW, barH, 4); ctx.fill();
          var fw = barW * (featR.trust / 100);
          if (fw > 0) { ctx.fillStyle = '#e8879e'; roundRect(barX, topY, Math.max(barH, fw), barH, 4); ctx.fill(); }
          ctx.strokeStyle = 'rgba(255,185,200,.3)'; ctx.lineWidth = 1; roundRect(barX, topY, barW, barH, 4); ctx.stroke();
          ctx.fillStyle = '#ffb9c8'; drawTinyHeart(barX + barW + 8, topY + barH / 2, 4.6);
        }
        rescueScreenPos = { x: rx, y: pcy, r: rr };
      } });
...
function renderPlay(now) {
  ...
  for (var y = 0; y < ROWS; y++) for (var x = 0; x < COLS; x++) {
    var cell = grid[y][x];
    var cx = GRID_X + x * CELL + CELL / 2, cy = GRID_Y + y * CELL + CELL / 2;
    if (isMaskedCell(curMask, x, y)) { // the level's fixed architecture — reads as room decor, not an empty gap
      ctx.fillStyle = 'rgba(40,28,58,.5)'; roundRect(cx - CELL / 2 + 6, cy - CELL / 2 + 6, CELL - 12, CELL - 12, 16); ctx.fill();
    ...
    var tl = curTiles[tkey(x, y)];
    if (tl) {
      if (tl.t === 'yarn') { drawYarnTile(cx, cy, tl.n); continue; }
      if (tl.t === 'spawn') { drawToyboxTile(cx, cy); continue; }
      if (tl.t === 'quilt') drawQuiltTile(cx, cy, tl.n);
      else if (tl.t === 'moon') drawMoonTile(cx, cy, now);
      else if (tl.t === 'draft') drawDraftTile(cx, cy, now);
    }
