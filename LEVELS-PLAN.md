...
| 8 | **The stairs** | a step in the floor | one row shifts sideways by 1 after every merge | a moving board — re-read every turn | 32 |
...
- **D1 · Blocker engine (#52) [Opus 4.8]** — generalize the board cell from `{pet, blocked}` to typed tiles: `quilt(n)`, `yarn(n)`, `moonbeam`, `draft`, `spawner(breed)`, `stairs(row)`. Hook into `settleAndRefill` (gravity must respect them), `resolveMerge` (merges clear adjacent quilt/yarn, moonbeam doubles, spawner emits), and **mirror every one inside `computeBotPar`** or par breaks. Render each as cozy house art. Feel risk = high → Opus.
